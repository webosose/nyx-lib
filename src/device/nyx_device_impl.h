...
#define CHECK_DEVICE(_d) {if (0 == _d) { \
    nyx_error (MSGID_NYX_DEVICE_INVALID_HANDLE, 0, "invalid device handle"); \
    return NYX_ERROR_INVALID_HANDLE; \
}}
#define CHECK_DEVICE_TYPE(_d,_t) {if ((_d)->type != _t) { \
    nyx_warn (MSGID_NYX_DEVICE_WRONG_DEVICE, 0, "wrong device type"); \
    return NYX_ERROR_WRONG_DEVICE_TYPE; \
}}
#define CHECK_DEVICE_METHOD(_d,_method) {if (0 == _d->method_ptr[_method]) { \
    nyx_debug ("method " #_method " not implemented"); \
...
#define CHECK_EVENT(_e) {if (0 == _e) { \
    nyx_warn (MSGID_NYX_DEVICE_INVALID_EVENT, 0, "invalid event handle"); \
    return NYX_ERROR_INVALID_HANDLE; \
}}
#define CHECK_EVENT_TYPE(_e,_t) {if ((_e)->type != _t) { \
    nyx_warn (MSGID_NYX_DEVICE_WRONG_EVENT_TYPE, 0, "wrong event type"); \
    return NYX_ERROR_WRONG_DEVICE_TYPE; \
}}
